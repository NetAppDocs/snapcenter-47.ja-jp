...
+
image::../media/nss_license_selection.gif[ライセンス]
...
NetApp Support Siteのクレデンシャルと SnapCenter ライセンスのシリアル番号を SnapCenter の GUI に入力しない場合や、 SnapCenter からNetApp Support Siteにインターネットにアクセスできない場合は、ネットアップライセンスファイル（ NLF ）を生成できます。 次に、 SnapCenter ホストからアクセス可能な場所にファイルをダウンロードして保存します。
...
. [Software]ページの[License]セクションで、*[Add]*（）をクリックしますimage:../media/add_policy_from_resourcegroup.gif["リソースグループからポリシーを追加"]。
